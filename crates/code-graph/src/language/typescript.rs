...
impl crate::language::Language for TypeScriptLanguage {
...
"#;
        ...
        run_query(query_str, tree_sitter_typescript::LANGUAGE_TSX.into(), tree, source, &mut |captures, source| {
    ...
    fn query_classes(&self, tree: &tree_sitter::Tree, source: &[u8], file_path: &str) -> Vec<GraphNode> {
        let query_str = r#"
(class_declaration name: (type_identifier) @class.name extends: (extends_clause (type_identifier) @class.parent)?) @class.def
...
"#;
        ...
        run_query(query_str, tree_sitter_typescript::LANGUAGE_TSX.into(), tree, source, &mut |captures, source| {
            ...
            for cap in captures {
                ...
                match cap.name {
                    "class.name" => { name = text.to_string(); node_type = NodeType::Struct; sl = row; sc = col; }
                    "iface.name" => { name = text.to_string(); node_type = NodeType::Struct; sl = row; sc = col; }
                    "enum.name" => { name = text.to_string(); node_type = NodeType::Struct; sl = row; sc = col; }
                    "type.name" => { name = text.to_string(); node_type = NodeType::TypeAlias; sl = row; sc = col; }
                    "class.parent" => parent = text.to_string(),
                    "class.def" | "iface.def" | "enum.def" | "type.def" => { el = erow; ec = ecol; }
                    _ => {}
                }
            }
    ...
    fn query_imports(&self, tree: &tree_sitter::Tree, source: &[u8], file_path: &str) -> Vec<GraphNode> {
        let query_str = r#"
(import_statement source: (string) @imp.source import_clause: (import_clause (identifier)? @imp.default (named_imports (import_specifier name: (identifier) @imp.name)*)?)?) @imp.stmt
"#;
        let mut nodes = Vec::new();
        run_query(query_str, tree_sitter_typescript::LANGUAGE_TSX.into(), tree, source, &mut |captures, source| {
            let mut name = String::new();
            ...
            for cap in captures {
                ...
                match cap.name {
                    ...
                    "imp.default" | "imp.name" => { name = text.to_string(); sl = row; sc = col; }
                    _ => {}
                }
            }
            ...
            if !display.is_empty() {
                Some(GraphNode {
                    ...
                    metadata: serde_json::json!({"from": from, "imported_name": name}),
                })
            } else { None }
        }).iter().for_each(|n| nodes.push(n.clone()));
        nodes
...
"#;
        ...
        run_query(query_str, tree_sitter_typescript::LANGUAGE_TSX.into(), tree, source, &mut |captures, source| {
            ...
            for cap in captures {
                let text = cap.node.utf8_text(source).unwrap_or("");
                match cap.name {
                    "call.callee" | "call.method" | "new.class" => callee = text.to_string(),
                    _ => {}
                }
            }
            if !callee.is_empty() {
                let caller = make_node_id(file_path, "__enclosing__", "Function", 1);
                let target = make_node_id(file_path, &callee, "Function", 1);
                Some(GraphEdge { source_id: caller, target_id: target, edge_type: EdgeType::Calls, weight: 0.8, target_name: String::new() })
            } else { None }
    ...
    fn query_inheritance(&self, tree: &tree_sitter::Tree, source: &[u8], file_path: &str, _file_nodes: &[GraphNode]) -> Vec<GraphEdge> {
        let query_str = r#"
(class_declaration name: (type_identifier) @class.name extends: (extends_clause (type_identifier) @class.parent)+) @class.ext
(interface_declaration name: (type_identifier) @iface.name extends: (extends_type_clause (type_identifier) @iface.parent)+) @iface.ext
"#;
        let mut edges = Vec::new();
        run_query(query_str, tree_sitter_typescript::LANGUAGE_TSX.into(), tree, source, &mut |captures, source| {
            ...
            for cap in captures {
                let text = cap.node.utf8_text(source).unwrap_or("");
                match cap.name {
                    "class.name" | "iface.name" => child = text.to_string(),
                    "class.parent" | "iface.parent" => parents.push(text.to_string()),
                    _ => {}
                }
            }
            if !child.is_empty() {
...
"#;
        ...
        run_query(query_str, tree_sitter_typescript::LANGUAGE_TSX.into(), tree, source, &mut |captures, source| {
            ...
            if !name.is_empty() {
                ...
                let tgt = make_node_id(file_path, &name, "Variable", 1);
                Some(GraphEdge { source_id: src, target_id: tgt, edge_type: EdgeType::References, weight: 0.5, target_name: String::new() })
            } else { None }
        }).iter().for_each(|e| edges.push(e.clone()));
        edges
    }
