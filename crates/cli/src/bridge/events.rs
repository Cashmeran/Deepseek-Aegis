...
pub enum ClientEvent {
    ...
    McpElicitationRequest { request: crate::bridge::types::ElicitationRequest },
    ...
    McpAuthRedirect { redirect: crate::bridge::types::McpAuthRedirect },
    /// MCP operation failed and should be surfaced in the MCP config UI.
    McpOperationError { error: crate::bridge::types::McpOperationError },
    /// A prompt turn completed successfully.
    TurnComplete { terminal_reason: Option<crate::bridge::types::TerminalReason> },
    /// `cancel` notification was accepted by the bridge.
    TurnCancelled,
    /// A prompt turn failed with an error.
    TurnError { message: String, terminal_reason: Option<crate::bridge::types::TerminalReason> },
    ...
    SessionsListed { sessions: Vec<crate::bridge::types::SessionListEntry> },
    /// Startup update check found a newer published version.
    UpdateAvailable { latest_version: String, current_version: String },
    /// Startup status check detected degraded/outage conditions.
    ...
    AuthCompleted { conn: Rc<crate::bridge::client::AgentConnection> },
    /// /logout completed via `claude auth logout`.
    LogoutCompleted,
    /// Status snapshot received from bridge (account info).
    StatusSnapshotReceived { session_id: String, account: crate::bridge::types::AccountInfo },
