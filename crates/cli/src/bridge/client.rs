...
use crate::bridge::wire::{BridgeCommand, CommandEnvelope, EventEnvelope, SessionLaunchSettings};
...
impl AgentConnection {
    ...
    pub fn prompt_with_images(
        ...
        session_id: String,
        text: String,
        images: Vec<crate::app::tools::clipboard_image::ImageAttachment>,
        ...
        for img in images {
            if let Err(reason) =
                crate::app::tools::clipboard_image::validate_image(&img.data, &img.mime_type)
            {
                tracing::warn!("prompt_with_images: skipping invalid image: {reason}");
                continue;
            }
            chunks.push(crate::bridge::types::PromptChunk {
                kind: "image".to_owned(),
    ...
    pub fn set_mcp_servers(
        &self,
        session_id: String,
        servers: std::collections::BTreeMap<String, crate::bridge::types::McpServerConfig>,
    ) -> anyhow::Result<()> {
        self.send(CommandEnvelope {
            request_id: None,
...
mod tests {
    ...
    use crate::bridge::types::ElicitationAction;
    ...
    fn set_mcp_servers_sends_bridge_command() {
        ...
        let servers = BTreeMap::from([(
            "notion".to_owned(),
            crate::bridge::types::McpServerConfig::Http {
