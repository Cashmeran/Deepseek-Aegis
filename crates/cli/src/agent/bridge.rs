...
const BRIDGE_RUNTIME_ENV_VAR: &str = "AEGIS_AGENT_BRIDGE_NODE";
const MAX_BRIDGE_EXE_ANCESTORS: usize = 8;
#[cfg(windows)]
const RENAMED_BRIDGE_RUNTIME_FILE_NAME: &str = "aegis-bridge-node.exe";
#[cfg(not(windows))]
const RENAMED_BRIDGE_RUNTIME_FILE_NAME: &str = "aegis-bridge-node";
...
impl BridgeLauncher {
    ...
    pub fn command(&self, bridge_diagnostics_enabled: bool) -> Command {
        ...
        cmd.env("AEGIS_BRIDGE_DIAGNOSTICS", if bridge_diagnostics_enabled { "1" } else { "0" });
...
impl<'a> BridgeScriptResolver<'a> {
    fn from_process(explicit_script: Option<&'a Path>) -> Self {
        Self {
            explicit_script,
            env_script: std::env::var_os("AEGIS_AGENT_BRIDGE").map(PathBuf::from),
    ...
    fn resolve(&self) -> anyhow::Result<PathBuf> {
        ...
        Err(anyhow::anyhow!(
            "bridge script not found near the installed executable. lookup mode {:?} checked: {}. expected bundled `agent-sdk/dist/bridge.js`; debug builds also check repo-local fallbacks. set AEGIS_AGENT_BRIDGE to override.",
            self.lookup_mode(),
...
mod tests {
    ...
    fn resolver_fixture() -> ResolverFixture {
        ...
        let installed_exe =
            dir.path().join("package").join("vendor").join("x86_64").join("aegis");
        let unbundled_exe =
            dir.path().join("other").join("vendor").join("x86_64").join("aegis");
        let cargo_target_exe =
            dir.path().join("manifest").join("target").join("debug").join("aegis");
    ...
    fn test_runtime_contents() -> &'static str {
        "@echo off\r\necho script=%~f1\r\necho diag=%AEGIS_BRIDGE_DIAGNOSTICS%\r\necho diagnostics-stderr 1>&2\r\n"
    }

    #[cfg(not(windows))]
    fn test_runtime_contents() -> &'static str {
        "#!/bin/sh\nprintf 'script=%s\\n' \"$1\"\nprintf 'diag=%s\\n' \"$AEGIS_BRIDGE_DIAGNOSTICS\"\nprintf 'diagnostics-stderr\\n' >&2\n"
    }
