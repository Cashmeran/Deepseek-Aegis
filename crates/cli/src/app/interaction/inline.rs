...
use super::super::{App, FocusTarget, InvalidationLevel, MessageBlock, ToolCallInfo};
use crossterm::event::{KeyCode, KeyEvent};

pub(crate) fn focused_interaction_id(app: &App) -> Option<&str> {
...
pub(crate) fn focused_interaction(app: &App) -> Option<&ToolCallInfo> {
...
pub(crate) fn get_focused_interaction_tc(app: &mut App) -> Option<&mut ToolCallInfo> {
    let tool_id = focused_interaction_id(app)?;
    let (mi, bi) = app.tool_call_index.get(tool_id).copied()?;
...
pub(crate) fn focused_interaction_dirty_idx(app: &App) -> Option<(usize, usize)> {
    focused_interaction_id(app).and_then(|tool_id| app.lookup_tool_call(tool_id))
}

pub(crate) fn invalidate_if_changed(
    app: &mut App,
...
pub(crate) fn set_interaction_focused(app: &mut App, queue_index: usize, focused: bool) {
...
pub(crate) fn focused_interaction_is_active(app: &App) -> bool {
...
pub(crate) fn clear_inline_interaction_focus(app: &mut App) {
    ...
    if changed || matches!(app.focus_owner(), super::super::FocusOwner::Permission) {
        app.release_focus_target(FocusTarget::Permission);
        app.normalize_focus_stack();
    }
}

pub(crate) fn focus_next_inline_interaction(app: &mut App) {
...
pub(crate) fn normalize_pending_interaction_queue(app: &mut App) {
    ...
    if changed {
        let permission_has_focus = matches!(app.focus_owner(), super::super::FocusOwner::Permission);
        for idx in 0..app.pending_interaction_ids.len() {
            set_interaction_focused(app, idx, permission_has_focus && idx == 0);
        }
    }
    if matches!(app.focus_owner(), super::super::FocusOwner::Permission) {
        app.claim_focus_target(FocusTarget::Permission);
        app.normalize_focus_stack();
    }
}

pub(crate) fn pop_next_valid_interaction_id(app: &mut App) -> Option<String> {
    normalize_pending_interaction_queue(app);
    (!app.pending_interaction_ids.is_empty()).then(|| app.pending_interaction_ids.remove(0))
}

pub(crate) fn handle_interaction_focus_cycle(
    app: &mut App,
    key: KeyEvent,
