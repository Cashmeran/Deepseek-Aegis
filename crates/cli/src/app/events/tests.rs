    ...
    fn is_update_notice_message(msg: &ChatMessage) -> bool {
        matches!(msg.role, MessageRole::System(Some(SystemSeverity::Warning)))
            && first_block_text(msg)
                .contains("Upgrade to latest version via cargo install aegis-cli.")
    }
    ...
    fn update_available_pushes_warning_system_message_with_versions_and_install_command() {
        ...
        assert_eq!(
            ...
            "Update available: current v0.2.0, latest v0.3.0. Upgrade to latest version via cargo install aegis-cli."
        );
    ...
    fn update_available_persists_across_connected_session_reset() {
        ...
        assert_eq!(
            ...
            "Update available: current v0.11.1, latest v0.11.2. Upgrade to latest version via cargo install aegis-cli."
        );
        assert_eq!(
            app.update_notice
                .as_ref()
