...
const AEGIS_DISABLE_1M_CONTEXT: &str = "AEGIS_DISABLE_1M_CONTEXT";
...
pub fn disable_1m_context(document: &Value) -> Result<bool, ()> {
    match read_json_path(document, &["env", AEGIS_DISABLE_1M_CONTEXT]) {
        None => Ok(false),
...
pub fn set_disable_1m_context(document: &mut Value, disabled: bool) {
    if disabled {
        set_json_path(
            document,
            &["env", AEGIS_DISABLE_1M_CONTEXT],
            Value::String("1".to_owned()),
        );
    } else {
        remove_json_path(document, &["env", AEGIS_DISABLE_1M_CONTEXT]);
    }
}
