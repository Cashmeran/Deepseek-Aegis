...
function App() {
  ...
  const filtered = searchQuery ? list.filter(s => (s.title || s.id).toLowerCase().includes(searchQuery.toLowerCase())) : list;
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: sideCollapsed ? '0px 1fr' : '260px 1fr', gridTemplateRows: '1fr', height: '100vh', transition: 'grid-template-columns 0.2s' }}>
      {/* Sidebar */}
      {!sideCollapsed && (
        <div className="sidebar">
          <div className="side-head">
            <button className="new-btn" onClick={() => setShowNewModal(true)}>+ 新建会话</button>
            <button className="icon-btn" onClick={() => setSideCollapsed(true)} title="收起侧边栏">
          ...
          <div className="search-row">
            <div className="input">
              ...
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="搜索会话..." />
            </div>
          </div>
          <div className="session-list">
            {filtered.map(s => (
              <div key={s.id}
                className="tree-item session-item"
                style={{ background: s.id === activeSessionId ? 'var(--accent-soft)' : undefined, color: s.id === activeSessionId ? 'var(--accent)' : undefined }}
                onClick={() => setActiveSessionId(s.id)}>
                <span className="session-dot" style={{ background: s.status === 'running' ? 'var(--accent)' : s.status === 'completed' ? 'var(--success)' : s.status === 'error' ? 'var(--danger)' : 'var(--muted)' }}/>
                <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.title || s.id}</span>
                <button className="icon-btn" style={{ width: 24, height: 24 }} onClick={e => { e.stopPropagation(); handleDelete(s.id); }}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6L6 18M6 6l12 12"/></svg>
                </button>
              </div>
            ))}
            {filtered.length === 0 && <div className="tree-empty">暂无会话</div>}
          </div>
          <div style={{ padding: '8px 12px', borderTop: '1px solid var(--border)', fontSize: 11, color: 'var(--muted)' }}>
            {connected ? '已连接' : '未连接'} · {providerConfigs.deepseek.model}
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        ...
        {sideCollapsed && (
          <button className="icon-btn" style={{ position: 'absolute', top: 12, left: 12, zIndex: 10 }} onClick={() => setSideCollapsed(false)}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 18l6-6-6-6"/></svg>
          </button>
        )}

        {/* Thread */}
        <div className="thread" style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
            ...
            {activeSession ? (
              <>
                {activeSession.messages.map((msg: Record<string, unknown>, i: number) => {
                  ...
                  if (t === "user_prompt") return (
                    <div key={i} className="user-msg" style={{ marginBottom: 16 }}>
                      <div className="user-msg-inner"><MDContent text={String(msg.prompt ?? "")}/></div>
                    </div>
                  );
                  if (t === "assistant") return (
                    <div key={i} className="assistant-msg" style={{ marginBottom: 16 }}>
                      <MDContent text={String(msg.text ?? "")}/>
                    </div>
                  );
                  if (t === "thinking") return (
                    <details key={i} style={{ marginBottom: 8, fontSize: 12, color: 'var(--muted)' }}>
                      <summary style={{ cursor: 'pointer', fontStyle: 'italic' }}>Thinking...</summary>
              ...
              <div className="splash">
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 8 }}>Aegis Desktop</div>
                  <div style={{ fontSize: 14, color: 'var(--muted)', marginBottom: 24 }}>
                    {connected ? '后端已连接' : '未连接'} · {providerConfigs.deepseek.apiKey ? providerConfigs.deepseek.model : '未配置 Key'}
                  </div>
