...
async fn stream_model(
  ...
  match provider.provider {
    ...
    ProviderKind::DeepSeek => stream_deepseek(app, state, session_id, provider).await,
  }
}
      ...
      for line in event.lines() {
        ...
        if let Some(delta) = payload.pointer("/choices/0/delta") {
          if let Some(content) = delta.get("content").and_then(Value::as_str) {
            ...
            assistant_text.push_str(content);
            emit_stream_event(app, session_id, "content_block_delta",
              Some(json!({ "type": "text_delta", "text": content })))?;
          }
          ...
          if let Some(reasoning) = delta.get("reasoning_content").and_then(Value::as_str) {
            if !reasoning.is_empty() {
              emit_stream_event(app, session_id, "content_block_delta",
                Some(json!({ "type": "reasoning_delta", "text": reasoning })))?;
            }
          }
          if let Some(calls) = delta.get("tool_calls").and_then(Value::as_array) {
            for call in calls {
